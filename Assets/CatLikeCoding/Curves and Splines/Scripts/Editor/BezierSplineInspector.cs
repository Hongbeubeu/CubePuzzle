...
namespace com.curve
{
    ...
    public class BezierSplineInspector : Editor
    {
        ...
        private void ShowDirections()
        {
            ...
            int step = stepsPerCurve * spline.CurveCount;
            for (int i = 1; i <= step; i++)
            {
                point = spline.GetPoint(i / (float)step);
                Handles.DrawLine(point, point + spline.GetDirection(i / (float)step) * directionScale);
            }
        }
